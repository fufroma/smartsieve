...
class Script {
  ...
  function updateScript () {
	...
	if (!$vacation['text']){
            if (SmartSieve::getConf('vacation_text', '') == '') {
        ...
        if (!$vacation['days']){
            if (SmartSieve::getConf('require_vacation_days')) {
                if (SmartSieve::getConf('vacation_days') === null){
	...
	if ((!is_array($vacation['addresses']) || empty($vacation['addresses'][0])) 
            && SmartSieve::getConf('require_vacation_addresses')) {
